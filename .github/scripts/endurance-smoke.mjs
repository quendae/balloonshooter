...
async function verifyWeatherStates(page) {
  const weatherCases = [
    ['clear', '☀ CLEAR', 'artifacts/endurance-weather-clear.png'],
    ['rain', '☂ RAIN', 'artifacts/endurance-weather-rain.png'],
    ['snow', '❄ SNOW', 'artifacts/endurance-weather-snow.png'],
    ['frost', '❄ FROST', 'artifacts/endurance-weather-frost.png'],
  ];

  for (const [target, badgeText, screenshotPath] of weatherCases) {
    await page.evaluate((nextWeather) => {
      const game = window.__enduranceGame;
      const previous = nextWeather === 'clear' ? 'rain' : game.weatherState.current;
      game.weatherState = {
        ...game.weatherState,
        previous,
        current: nextWeather,
    ...
    await page.waitForFunction((expected) => document.querySelector('#enduranceWeatherBadge')?.dataset.weather === expected, target);
    assert(await page.locator('#enduranceWeatherBadge').isVisible(), `${target} weather badge should be visible`);
    assert((await page.locator('#enduranceWeatherBadge').textContent()) === badgeText, `${target} weather badge text mismatch`);
    ...
    assert(state.weather === target, `${target} weather should stay selected during visual probe`);
    assert(state.weatherProgress > 0 && state.weatherProgress < 1, `${target} weather crossfade should be mid-transition, got ${state.weatherProgress}`);
    await page.screenshot({ path: screenshotPath, fullPage: true });
  }
}

async function verifyFrostProjectile(page) {
  await page.evaluate(() => {
    const game = window.__enduranceGame;
    game.weatherState = {
      ...game.weatherState,
  ...
  assert(frostShot.projectile?.color === 2, 'Frost projectile should preserve queued color');
  assert(frostShot.projectile?.weatherType === 'frost', 'Frost projectile should snapshot weather type');
  assert(Math.abs(frostShot.projectile?.collisionScale - .82) < 1e-9, `Frost projectile scale ${frostShot.projectile?.collisionScale}`);

  await page.evaluate(() => {
    const game = window.__enduranceGame;
    game.weatherState = { ...game.weatherState, previous: 'frost', current: 'clear' };
    game.emitState();
  });
...
async function verifyDesktop(browser) {
  ...
  assert(await page.locator('#enduranceWeatherBadge').isVisible(), 'Endurance weather badge should be visible');
  assert((await page.locator('#enduranceWeatherBadge').getAttribute('data-weather')) === 'clear', 'Endurance should start in clear weather');
  ...
  await page.evaluate(() => {
    ...
    // Synthetic time jumps are UI probes. Freeze RAF so the weather scheduler
    // cannot consume 400s of history and overwrite the callout under test.
    game.setPaused(true);
    game.emitState();
  });
  ...
  await verifyWeatherStates(page);
  await verifyFrostProjectile(page);
...
try {
  ...
  console.log('OK: Endurance v2 geometry, pressure, palette stages, specials, shallow aim, dynamic weather, Frost collision, loss/retry and mobile layout passed');
} finally {
  await browser.close();
}
